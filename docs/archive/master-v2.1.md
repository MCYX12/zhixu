...
版本：v2.1 · 2026-09-13。本文是实施、验收和范围调整的总依据。原始方案归档于 `docs/archive/mac_m4_local_first_ai_v1.original.md`，实际进度见 `docs/implementation-status.md`。计划能力不代表已经实现。

## 1. 目标与用户确认

硬件：Mac mini M4、32GB。当前不使用 NAS，原始资料、索引和会话全部保存在本机 SSD。用户已确认：默认全部本地，联网和云端后续手动开启。

当前交付以企业小范围试点为目标：美观的「知序」工作台、本机文档上传、带用户权限的资料问答、来源引用、会话历史与运行状态。随后优先完善检索质量、账号治理、多设备部署和运维；联网和云端仍由用户后续决定。模型和后端可替换，但须适配和回归测试。
...
M1～M3 构成当前本机企业试点版本，不代表生产部署全部完成。接下来优先 M4 检索评估和 M5 运维；NAS 不作为任何当前里程碑的前置条件。mock 不能替代真实模型性能或多设备验收。未完成项必须留在状态文档中。
